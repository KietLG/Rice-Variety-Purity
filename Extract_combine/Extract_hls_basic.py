...
def extract_basic_features(image):
    """
    Trích xuất 30 đặc trưng cơ bản (enhanced) gồm:
      - 14 đặc trưng hình học của đối tượng (dựa trên Otsu thresholding).
...
def extract_hls_features(image):
    ...
    Trích xuất 21 đặc trưng từ không gian HLS:
      Với mỗi kênh (H, L, S) tính: mean, std, skewness, kurtosis, median, min, max.
    """
...
def extract_features(image):
    """Kết hợp đặc trưng basic và HLS thành một vector đặc trưng."""
    return extract_basic_features(image) + extract_hls_features(image)
