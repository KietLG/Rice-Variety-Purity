...
def extract_ycrcb_features(image):
    ...
    Trích xuất 21 đặc trưng từ không gian YCrCb:
      Với mỗi kênh (Y, Cr, Cb) tính: mean, std, skewness, kurtosis, median, min, max.
    """
...
def extract_all_features(image):
    """Ghép nối đặc trưng basic và YCrCb thành một vector đặc trưng."""
    feat_basic = extract_basic_features(image)   
    feat_ycrcb = extract_ycrcb_features(image)   
    return feat_basic + feat_ycrcb
